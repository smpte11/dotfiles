...
-- This is a jj-colocated repo, so git HEAD is detached and octo's git-branch
...
-- Same-repo: fetch + `jj new <head>@origin`. Fork: fall back to
-- `gh pr checkout` then `jj git import` to sync git refs into jj.
